...
`,
  ...
  // ============================================
  // UPDATED: Listen for messages from the Bridge (Isolated World)
  // ============================================
  ...
  window.addEventListener('message', (event) => {
    ...
    // Check if message is from our bridge
    const msg = event.data;
    if (msg && msg.source === 'COMET_BRIDGE_TO_MAIN') {
      const { type, payload } = msg;

      // Map Chrome messages to internal functions
      if (type === 'TOGGLE_OVERLAY') {
        toggleOverlay(true); // broadcast=true
      } else if (type === 'SHOW_OVERLAY') {
        showOverlay(true);
      } else if (type === 'HIDE_OVERLAY') {
        hideOverlay(true);
      } else if (type === 'MCP_CALL') {
        window.CometDevTools.MCP.callTool(payload.tool, payload.args);
      }
    }
  });
